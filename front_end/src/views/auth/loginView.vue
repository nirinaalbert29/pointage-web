    ...
    <div class=" flex flex-col sm:w-full  items-start">
        <div class="flex flex-col justify-centerlear-left sm:py-4 sm:px-5 rounded-xl  ">
            ...
            <div class=" border border-gray-300 rounded-lg px-2 py-1 flex flex-row">
                ...
                <input v-model="user.email" type="text" placeholder="Adresse email" class=" bg-transparent outline-none ml-2  py-1">
            </div>
            <div class=" border border-gray-300 rounded-lg px-2 py-1 flex flex-row mt-4">
                <svg class="w-5" viewBox="0 0 24 24">
            ...
            <div class=" rounded-lg px-2 bg-indigo-500 flex flex-row mt-4 border border-indigo-500 py-1">
                <button @click="connection()" class="  text-white text-lg w-full rounded-lg uppercase"> Se Connecter</button>
            </div>
    ...
    methods: {
        ...
        async connection() {
            ...
            try {
                ...
                if (_r.status) {
                    console.log( _r.data.data[0])
                    this.$store.state.data.user=_r.data.data[0]
                    // window.location = '/'    
                }
            } catch (e) {
